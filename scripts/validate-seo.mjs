...
for (const slug of slugs) {
	...
	if (!fm.slug) {
		...
		errors.push(`slug "${fm.slug}"는 영문 소문자와 숫자, 하이픈만 허용`);
...
for (const name of seriesNames) {
	...
	try {
		...
		violations.push({ slug: `series/${name}`, errors: [`파일 파싱 실패: ${e.message}`] });
		continue;
	}
...
if (violations.length > 0) {
	...
	console.error(
		`\n총 ${violations.length}개 항목 위반. .agents/rules/seo.md 기준 (description ${DESC_MIN}~${DESC_MAX}자, title ${TITLE_MAX}자 이하).\n`
	);
	process.exit(1);
}
