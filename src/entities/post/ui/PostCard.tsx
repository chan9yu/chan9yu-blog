...
export function PostCard({ post, priority = false }: PostCardProps) {
	return (
		<Link
			...
			className="group bg-card border-border-subtle hover:border-accent/40 focus-visible:ring-ring flex h-full flex-col overflow-hidden rounded-lg border transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
		>
			...
			<div className="flex flex-1 flex-col gap-2 p-4.5">
				...
				<time className="text-muted-foreground text-12 block tabular-nums" dateTime={post.date}>
					{formatDate(post.date)}
				</time>
				<p className="text-muted-foreground leading-body text-13 line-clamp-3 text-pretty">{post.description}</p>
