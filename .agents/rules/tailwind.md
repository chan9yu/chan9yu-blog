...
**글자 크기 계단이 둘이다.** 이름 있는 계단(`text-chip` 11.5px에서 `text-base` 15.5px)과 정수 계단(`text-11`에서 `text-17`)이 0.5px 어긋난 채 공존한다. 이름 있는 계단이 101곳, 정수 계단이 21곳에서 쓰인다.

하나로 합치려면 둘 중 하나를 골라야 한다. 이름 있는 계단을 정수로 내리면 101곳이 0.5px 작아진다. 정수 계단을 올리면 21곳이 커지고, 그중 줄 높이 동반 값 없는 토큰을 쓰던 자리에 줄 높이가 새로 걸린다.
