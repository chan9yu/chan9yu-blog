...
**목표.** 이 저장소에서 실제로 깨지는 자리를 지킨다. 레이어 경계와 직접 만든 UI, 접근성, 저장소 규칙 위반 넷이다.

**진입점은 `main-harness` 스킬 하나다.** 코드를 만들거나 고치는 요청이면 종류를 가리지 않고 여기서 시작한다. 범위 판정과 에이전트 배분, 게이트 순서를 그 스킬이 정한다. 단순 질문과 문서 조회, 커밋과 배포만 하는 요청은 직접 답한다.

**에이전트 넷.** fsd-architect가 레이어 배치와 경계를 정하고, ui-builder가 토큰 위에 컴포넌트를 만들고, a11y-verifier가 접근성을 판정하고, code-reviewer가 규칙 위반을 잡는다. **만든 사람이 판정하지 않는다.** 이 분리가 하네스의 전부다.

**스킬 셋.** `main-harness`가 진입점이고, `primitive-build`가 프리미티브 구현 절차를, `a11y-dialog-check`가 대화상자 접근성 검증을 맡는다.

**변경 이력.**

| 날짜       | 변경 내용                          | 대상                                           | 사유                                                                                        |
| ---------- | ---------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 2026-08-27 | `main-harness` 오케스트레이터 추가 | skills/main-harness                            | 기능 요청에 자동으로 붙는 진입점이 없었다                                                   |
| 2026-08-27 | `fsd-nextjs` 스킬 삭제             | skills/fsd-nextjs                              | 일반 FSD 교과서 986줄이고, 레이어 이름에 밑줄을 붙이라며 이 저장소의 결정과 반대를 지시했다 |
| 2026-08-27 | 터치 대상 기준을 44px에서 24px로   | agents/a11y-verifier, skills/a11y-dialog-check | 44px은 AAA(2.5.5)다. AA(2.5.8)는 24px이고 예외가 넷 있다                                    |
| 2026-08-27 | 대화상자 검증 항목을 한 곳으로     | agents 둘, skills 둘                           | 같은 목록이 네 곳에 있어 네이티브 dialog 전환 뒤 서로 어긋났다                              |
